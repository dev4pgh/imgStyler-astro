...
const OverlaysPanel = () => {
    ...
    const {
        overlays, setOverlays, hasImage, setOverlayInteractionState, isCropping,
    ...
    const startOverlayDraw = (type) => {
        ...
        console.log(`Activating ${type} overlay drawing mode...`);
    ...
    return (
        <div className="p-2 space-y-4">
            ...
            <div className="space-y-3 border-t border-gray-300 dark:border-gray-600 pt-3 mt-3">
                ...
                <ul className="space-y-3 max-h-[calc(100vh-400px)] overflow-y-auto pr-1">
                                ...
                                {overlay.type === 'text' && (
                                    <div className="space-y-2">
                                        ...
                                        <div className="grid grid-cols-2 gap-2 items-center">
                                            ...
                                            <div>
                                                <label className="block text-xs mb-0.5 text-gray-600 dark:text-gray-300">Color:</label>
                                                <input
                                                    type="color"
                                                    value={overlay.color}
                                        ...
                                        <div>
                                            <label className="block text-xs mb-1 text-gray-600 dark:text-gray-300">Alignment:</label>
                                            <div className="flex space-x-1">
                                                {['left', 'center', 'right'].map(align => (
                                                    <button
                                                        key={align}
                                                        onClick={() => updateOverlayProperty(overlay.id, 'textAlign', align)}
                                                        className={`flex-1 px-2 py-1 text-xs rounded border transition-colors
                                                        ${overlay.textAlign === align
                                                                ? 'bg-blue-500 border-blue-600 text-white font-medium'
                                                                : 'bg-gray-200 border-gray-300 text-gray-700 hover:bg-gray-300 dark:bg-gray-600 dark:border-gray-500 dark:text-gray-200 dark:hover:bg-gray-500' // Inactive state
                                                            }
                                        ...
                                        <div>
                                            <label className="block text-xs mb-0.5 text-gray-600 dark:text-gray-300">Font:</label>
                                            <select
                                                value={overlay.fontFamily}
                                                onChange={(e) => updateOverlayProperty(overlay.id, 'fontFamily', e.target.value)}
                                                className={`w-full text-xs rounded px-2 py-1 border border-gray-300 dark:border-gray-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-1 focus:ring-blue-400 ${isDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                                                disabled={isDisabled}
